...
if ( ! class_exists( 'Unicamp_Woo' ) ) {
	class Unicamp_Woo {
		...
		public function initialize() {
			...
			add_filter( 'unicamp_login_url', [ $this, 'update_profile_url' ], 15 );
			add_filter( 'unicamp_register_url', [ $this, 'update_profile_url' ], 15 );
		...
		function get_mini_cart() {
			...
			$qty = 0;
			if ( function_exists( 'WC' ) && WC() && isset( WC()->cart ) && WC()->cart instanceof WC_Cart ) {
		...
		public function product_infinite_load() {
			...
			if ( 'current_query' === $source ) {
				$query_vars                = isset( $_POST['query_vars'] ) && is_array( $_POST['query_vars'] ) ? array_map( 'sanitize_text_field', $_POST['query_vars'] ) : array();
				$query_vars['paged']       = ! empty( $_POST['paged'] ) ? intval( $_POST['paged'] ) : 1;
				...
				$post_type      = ! empty( $query_vars['post_type'] ) ? sanitize_text_field( $query_vars['post_type'] ) : 'product';
				$posts_per_page = ! empty( $query_vars['posts_per_page'] ) ? intval( $query_vars['posts_per_page'] ) : - 1;
				$orderby        = ! empty( $query_vars['orderby'] ) ? sanitize_text_field( $query_vars['orderby'] ) : 'date';
				$order          = ! empty( $query_vars['order'] ) ? sanitize_text_field( $query_vars['order'] ) : 'DESC';
				$paged          = ! empty( $query_vars['paged'] ) ? intval( $query_vars['paged'] ) : 1;
